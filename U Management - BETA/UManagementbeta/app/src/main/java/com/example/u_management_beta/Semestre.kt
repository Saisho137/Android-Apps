...
class Semestre : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val irMateria = findViewById<Button>(R.id.switchMateria)
        ...
        btnCalculo.setOnClickListener{
            ...
            try {
                for (i in arrayNotas.indices){
                    ...
                    temp += arrayPorcentajes[i]
                }
                acumuladorTotal += acumuladorNota
                acumuladorTotal /= temp
                val prit = round(acumuladorTotal* 100) / 100
                resultado.text = "$prit"
            } catch (e: IndexOutOfBoundsException){
                val alerta = AlertDialog.Builder(this)
                    .setTitle("Error!").setMessage("Aún no has guardado ninguna Nota!")
